...
class Registro:
    ...
    def editar(self, id):
        cursor.execute(
            "UPDATE Diario SET registro = ? WHERE id = ?;", (self._texto, id)
        )
        conexao.commit()
...
def menu_agenda():
    while True:
        ...
        if opcao == "Adicionar registro":
            ...
            numero_do_id = int(
                questionary.text(
                    "Qual o ID do registro que você deseja deletar? ",
                    validate=isdigit,
                    qmark="",
                ).ask()
            )
            if verificar_id(numero_do_id) == True:
                ...
                print(
                    marcar_textos(
                        "Não há nenhum registro com esse ID.", "vermelho", True
                    )
                )
                sleep(2)
                system("cls")
            ...
            numero_do_id = int(
                questionary.text(
                    "Qual o ID do registro que você deseja editar? ",
                    validate=isdigit,
                    qmark="",
                ).ask()
            )
